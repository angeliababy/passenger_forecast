...
def cnnLayer(x, is_training):
    ...
    out = tf.reshape(out, [-1, size, size])
...
def fusion(x1, x2, x3, is_training):
    ...
    W1 = weightVariable([size, size])
    W2 = weightVariable([size, size])
    W3 = weightVariable([size, size])
    # 短期、周期、趋势融合
    out = tf.multiply(out1,W1) + tf.multiply(out2,W2) + tf.multiply(out3,W3)
    return out

# 外部条件融合层
def exnetwork(ex, x1, x2, x3, is_training):
    # 全连接层
    Wf = weightVariable([ex.shape[1].value, size])
    bf = biasVariable([size])
    dense1 = tf.nn.relu(tf.matmul(ex, Wf) + bf)
    # 全连接层
    Wf = weightVariable([size, size*size])
    bf = biasVariable([size*size])
    dense2 = tf.nn.relu(tf.matmul(dense1, Wf) + bf)
    dense2 = tf.reshape(dense2, [-1, size, size])

    # 融合内、外部条件
    W1 = weightVariable([size, size])
    W2 = weightVariable([size, size])
    out = fusion(x1, x2, x3, is_training)
    # 内部、外部融合
    out = tf.multiply(out, W1) + tf.multiply(dense2, W2)
